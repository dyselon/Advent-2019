    ...
    pub fn step(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        match opcode.instruction {
            ...
            Instruction::JumpIfTrue => {
                let test = self.read_operand(opcode.left);
                let dest = self.read_operand(opcode.right);
                if test != 0 { self.ip = dest as usize; }
            }
            ...
            Instruction::LessThan => {
                let cmp = self.read_operand(opcode.left) < self.read_operand(opcode.right);
                self.write_operand(match cmp {
                    true => 1,
                    false => 0 
                });
            }
